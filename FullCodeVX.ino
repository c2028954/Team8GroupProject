...
float temperatureReadings[NUM_SAMPLES];
...
void loop() {
  ...
  if (IMU.accelerationAvailable()&&IMU.gyroscopeAvailable()) {
    ...
    if (abs(ax)>1.2 || abs(ay)>1.2 || abs(az)>1.2){
      LoRa.beginPacket();
      LoRa.println("unusual vibrations detected");
      LoRa.endPacket();
